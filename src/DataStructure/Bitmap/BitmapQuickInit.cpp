...
inline bool
dsa::BitmapQuickInit::isValid(Rank r) const {
    return r >= 0 and r < m_top;
}

inline bool
dsa::BitmapQuickInit::isErase(Rank r) const {
    return isValid(m_rank[r]) and ((m_stack[m_rank[r]] + r + 1) == 0);
}

inline bool
dsa::BitmapQuickInit::isRecord(Rank r) const {
    return isValid(m_rank[r]) and (m_stack[m_rank[r]] == r);
}

inline void
dsa::BitmapQuickInit::set(Rank r) {
    if (isRecord(r)) {
        return;
    }
    if (!isErase(r)) {
...
inline void
dsa::BitmapQuickInit::clear(Rank r) {
    if (isRecord(r)) {
